...
def EmbeddingSpace(tree: Tree, device, config: Config, train: bool, prototype_path: str = ''):
    ...
    if config.embedding_space._GEOMETRY == 'hyperbolic':
        ...
        logger.error(
            f'Embedding space geometry {config.embedding_space._GEOMETRY} not supported.')
    ...
    return EmbeddingSpace(tree=tree, config=config, train=train, prototype_path=prototype_path, device=device)
